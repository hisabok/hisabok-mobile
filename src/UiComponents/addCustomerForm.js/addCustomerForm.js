...
const AddCustomerScreen = ({ navigation }) => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        rent: Yup.number().required('Rent amount is required').positive('Rent amount must be positive')
    });
    ...
    const submitHandler = async () => {
        ...
        try {
            const payload = {
                customer_full_name: formValues.name,
                customer_mobile: formValues.mobile,
                email: formValues.email,
                hisab_name: formValues.name,
                amount_total_credit: formValues.rent,
                amount_total_payments: 0
            };
            
            // Console log the request object before sending it
            console.log('API Request Payload:', payload);

            const response = await createCustomer(payload);
            
            // Console log the full response object received from the API
            ...
            if (response.success) {
                ...
                showSnackbar(response.message || 'Failed to add customer.', 'error');
            }
        } catch (error) {
            console.error('API Error:', error);
            showSnackbar(error.response?.data?.message || 'Error adding customer.', 'error');
        }
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <Formik
                initialValues={{ name: '', email: '', mobile: '', rent: '' }}
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        ...
                        <View style={styles.modalButtons}>
                            ...
                            <TouchableOpacity style={[styles.modalButton, styles.confirmButton]} onPress={submitHandler}>
